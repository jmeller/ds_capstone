...
In total,there are ```r length(freq)``` items that are less sparse than ```r sparsity_threshold```, i.e., all of these words occur in at least ```r (1-sparsity_threshold)```% of the documents. 
```{r plot_data, echo=FALSE}
plot_data <- data.frame(term=names(freq),occurrences=freq)
plot <- data.frame(term=names(freq),occurrences=freq) %>%
  ggplot(aes(x=occurrences)) + geom_histogram() + theme_bw()
plot
```


```{r}
freq <- colSums(as.matrix(removeSparseTerms(dtm, 0.999)))
